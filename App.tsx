...
const AppContent: React.FC = () => {
    ...
    const handleOpenAddVoucher = (providerId: string) => {
    ...
    const handleAddProviderSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const nextOriginalId = providers.length > 0 ? Math.max(...providers.map(p => p.originalId)) + 1 : 1;
    ...
    const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                for (const [index, row] of json.slice(1).entries()) {
                    ...
                    let provider = await findProviderByOriginalId(parsedProviderId);
                    if (!provider) {
                        const newProviderName = `Provider ${parsedProviderId}`;
                        await addProvider({ name: newProviderName, originalId: parsedProviderId });
                        addToast('info', `Provider baru "${newProviderName}" dibuat otomatis.`);
                        provider = await findProviderByOriginalId(parsedProviderId);
                    }
                    
                    if (!provider) {
                        errorLog.push(`Baris ${index + 2}: Gagal membuat atau menemukan provider.`);
                        continue;
                    }
                    ...
                    await upsertVoucher(voucherData);
                    successCount++;
                }
        ...
        <div className="min-h-screen flex flex-col bg-light-bg dark:bg-dark-bg text-gray-800 dark:text-gray-200 transition-colors duration-300">
            ...
            <Modal isOpen={isVoucherFormOpen} onClose={() => { setIsVoucherFormOpen(false); setVoucherNameSuggestions([]); }} title={editingVoucher ? 'Edit Voucher' : 'Tambah Voucher Baru'}>
                <form onSubmit={handleVoucherFormSubmit} className="space-y-4">
                    ...
                    <div className="relative">
                        ...
                        <input name="name" value={voucherFormData.name || ''} onChange={handleVoucherFormChange} onBlur={() => setTimeout(() => setVoucherNameSuggestions([]), 200)} autoComplete="off" required className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border border-gray-300 dark:border-navy-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-500" />
                        {voucherNameSuggestions.length > 0 && (
                            <ul className="absolute z-10 w-full mt-1 bg-light-card dark:bg-navy-700 border border-gray-300 dark:border-navy-800 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                {voucherNameSuggestions.map(suggestion => (<li key={suggestion} onMouseDown={() => handleSuggestionClick(suggestion)} className="px-4 py-2 cursor-pointer hover:bg-gray-100 dark:hover:bg-navy-800">{suggestion}</li>))}
                            </ul>
                        )}
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="totalStock" type="number" value={voucherFormData.totalStock ?? ''} onChange={handleVoucherFormChange} required className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border border-gray-300 dark:border-navy-700 rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium">Sisa Stok</label>
                            <input name="remainingStock" type="number" value={voucherFormData.remainingStock ?? ''} onChange={handleVoucherFormChange} required className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border border-gray-300 dark:border-navy-700 rounded-lg" />
                        </div>
                         <div>
                            <label className="block text-sm font-medium">Harga Modal</label>
                            <input name="costPrice" value={formatNumberWithSeparators(voucherFormData.costPrice ?? '')} onChange={handleVoucherFormChange} required className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border border-gray-300 dark:border-navy-700 rounded-lg" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium">Harga Jual</label>
                            <input name="sellPrice" value={formatNumberWithSeparators(voucherFormData.sellPrice ?? '')} onChange={handleVoucherFormChange} required className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border border-gray-300 dark:border-navy-700 rounded-lg" />
                        </div>
                        <div className="col-span-2">
                            <label className="block text-sm font-medium">Rencana Tambah Stok</label>
                            <input name="plannedStock" type="number" value={voucherFormData.plannedStock ?? ''} onChange={handleVoucherFormChange} className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border border-gray-300 dark:border-navy-700 rounded-lg" />
                            {estimatedPlannedCost > 0 && (<p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Estimasi biaya: <strong>{formatCurrency(estimatedPlannedCost)}</strong></p>)}
                        </div>
                    </div>
                    <div className="pt-4 flex justify-end">
                        <button type="submit" className="px-6 py-2 bg-gold-500 text-navy-900 font-bold rounded-lg shadow-md hover:bg-gold-600">Simpan</button>
                    </div>
                </form>
            </Modal>
            ...
            <Modal isOpen={isAddProviderModalOpen} onClose={() => setIsAddProviderModalOpen(false)} title="Tambah Provider Baru">
                <form onSubmit={handleAddProviderSubmit} className="space-y-4">
                     <div>
                        <label className="block text-sm font-medium">Nama Provider</label>
                        <input name="providerName" required className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border rounded-lg" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium">URL Logo (Opsional)</label>
                        <input name="providerLogoUrl" type="url" placeholder="https://example.com/logo.png" className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border rounded-lg" />
                    </div>
                     <div className="pt-4 flex justify-end">
                        <button type="submit" className="px-6 py-2 bg-gold-500 text-navy-900 font-bold rounded-lg">Tambah</button>
                    </div>
                </form>
            </Modal>
            
            <Modal isOpen={!!stockToAdd.voucher} onClose={() => setStockToAdd({ voucher: null, quantity: '' })} title={`Tambah Stok: ${stockToAdd.voucher?.name}`}>
                <div className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium">Jumlah Tambahan</label>
                        <input type="number" value={stockToAdd.quantity} onChange={(e) => setStockToAdd(prev => ({ ...prev, quantity: e.target.value }))} autoFocus className="w-full px-3 py-2 bg-gray-100 dark:bg-navy-800 border rounded-lg" />
                        {stockToAdd.voucher?.plannedStock > 0 && <p className="text-xs mt-1">Rencana tambah: {stockToAdd.voucher.plannedStock} pcs.</p>}
                    </div>
                    <div className="pt-4 flex justify-end gap-4">
                        <button onClick={() => setStockToAdd({ voucher: null, quantity: '' })} className="px-6 py-2 rounded-lg bg-gray-300 dark:bg-navy-700">Batal</button>
                        <button onClick={handleConfirmAddStock} className="px-6 py-2 bg-gold-500 text-navy-900 font-bold rounded-lg">Tambah</button>
                    </div>
                </div>
            </Modal>

            <Modal isOpen={!!deletingVoucher} onClose={() => setDeletingVoucher(null)} title="Konfirmasi Hapus">
                <p>Yakin ingin menghapus voucher <strong>{deletingVoucher?.name}</strong>?</p>
                <div className="mt-6 flex justify-end gap-4">
                    <button onClick={() => setDeletingVoucher(null)} className="px-6 py-2 rounded-lg bg-gray-300 dark:bg-navy-700">NO</button>
                    <button onClick={confirmDeleteVoucher} className="px-6 py-2 bg-red-600 text-white font-bold rounded-lg">YES</button>
                </div>
            </Modal>
            
             <Modal isOpen={!!deletingProvider} onClose={() => setDeletingProvider(null)} title="Konfirmasi Hapus">
                <p>Yakin ingin menghapus <strong>{deletingProvider?.name}</strong>? Semua vouchernya akan terhapus.</p>
                <div className="mt-6 flex justify-end gap-4">
                    <button onClick={() => setDeletingProvider(null)} className="px-6 py-2 rounded-lg bg-gray-300 dark:bg-navy-700">NO</button>
                    <button onClick={confirmDeleteProvider} className="px-6 py-2 bg-red-600 text-white font-bold rounded-lg">YES</button>
                </div>
            </Modal>
            ...
            <Modal isOpen={isReportModalOpen} onClose={() => setIsReportModalOpen(false)} title={reportTitle}>
                ...
                <div className="mt-6 flex justify-end">
                    <button onClick={copyReportToClipboard} className="flex items-center gap-2 px-6 py-2 bg-gold-500 text-navy-900 font-bold rounded-lg"><Clipboard size={18} /> Copy</button>
                </div>
            </Modal>
            
            <Modal isOpen={isPrintChoiceModalOpen} onClose={() => setIsPrintChoiceModalOpen(false)} title="Pilih Jenis Resi">
                <div className="flex flex-col gap-4">
                    <p className="text-sm">Pilih jenis laporan untuk dicetak.</p>
                    <button onClick={() => handlePrintSelectedReceipt('complete')} className="w-full px-6 py-3 bg-navy-700 text-white font-bold rounded-lg">Laporan Lengkap</button>
                    <button onClick={() => handlePrintSelectedReceipt('short')} className="w-full px-6 py-3 bg-gold-500 text-navy-900 font-bold rounded-lg">Laporan Singkat</button>
                </div>
            </Modal>
                ...
                <div className="max-h-96 overflow-y-auto space-y-3 pr-2">
                    {activityLogs.length > 0 ? activityLogs.map(log => (
                        <div key={log.id} className="text-sm p-3 bg-gray-100 dark:bg-navy-800 rounded-lg">
                            ...
                            <p className="text-xs text-gray-500 mt-1">{new Date(log.timestamp).toLocaleString('id-ID')}</p>
                        </div>
                    )) : <p className="text-center text-gray-500">Belum ada aktivitas.</p>}
                </div>
            </Modal>

            <div aria-live="assertive" className="fixed inset-0 flex flex-col items-end px-4 py-6 pointer-events-none sm:p-6 space-y-4">
...
const App: React.FC = () => {
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-light-bg dark:bg-dark-bg text-lg">
                Loading Application...
            </div>
        );
    }
